...
// GenerateAllNumberVerificationCode 函数用于生成全数字的验证码。
//
// 输入参数：
//   - length: 验证码长度。
//
// 输出参数：
//   - string: 生成的验证码。
func GenerateAllNumberVerificationCode(length int, purpose string) string {
	// 计算最小值和最大值
	...
	// 检查验证码是否重复
	if GetCodeByKey(code, purpose) != "" {
		SysError("repeated verification code detected")

		// 如果重复，递归调用自身增加验证码长度
		return GenerateAllNumberVerificationCode(length+1, purpose)
	}
...
// GetCodeByKey 函数用于根据指定的键和目的获取验证码。
//
// 输入参数：
//   - key: 键值。
//   - purpose: 目的类型。
//
// 输出参数：
//   - string: 验证码内容，如果不存在或已过期则返回空字符串。
func GetCodeByKey(key string, purpose string) string {
	// 加锁保护并发访问
	verificationMutex.Lock()
	defer verificationMutex.Unlock()
