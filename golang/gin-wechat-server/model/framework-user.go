...
func (user *UserEntity) Authenticate() (err error) {
	...
	// 使用 GORM 查询用户信息
	err = DB.Where(UserEntity{UserName: user.UserName}).First(user).Error
	if err != nil {
		return err
	}
...
func GetUserByID(id int, selectAll bool) (*UserEntity, error) {
	// 如果传入的用户ID为0，则返回错误信息
	if id == 0 {
...
func GetByUserName(userName string, selectAll bool) (*UserEntity, error) {
	// 如果用户名为空，则返回错误信息
	if userName == "" {
