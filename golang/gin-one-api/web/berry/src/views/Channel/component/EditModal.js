...
const validationSchema = Yup.object().shape({
	...
	baseUrl: Yup.string().when("type", {
		...
		then: Yup.string().required("渠道API地址 不能为空"), // baseUrl 是必需的
		otherwise: Yup.string(), // 在其他情况下，base_url 可以是任意字符串
	}),
	modelMapping: Yup.string().test("is-json", "必须是有效的JSON字符串", function (value) {
		try {
...
const EditModal = ({ open, channelId, onCancel, onOk }) => {
	...
	const submit = async (values, { setErrors, setStatus, setSubmitting }) => {
		setSubmitting(true);
		if (values.baseUrl && values.baseUrl.endsWith("/")) {
			values.baseUrl = values.baseUrl.slice(0, values.baseUrl.length - 1);
		}
				...
				<Formik initialValues={initialInput} enableReinitialize validationSchema={validationSchema} onSubmit={submit}>
					{({ errors, handleBlur, handleChange, handleSubmit, isSubmitting, touched, values, setFieldValue }) => (
						<form noValidate onSubmit={handleSubmit}>
							...
							<FormControl fullWidth error={Boolean(touched.baseUrl && errors.baseUrl)} sx={{ ...theme.typography.otherInput }}>
								<InputLabel htmlFor="channel-base_url-label">{inputLabel.baseUrl}</InputLabel>
								<OutlinedInput
									id="channel-base_url-label"
									label={inputLabel.baseUrl}
									type="text"
								...
								{touched.baseUrl && errors.baseUrl ? (
									...
									<FormHelperText id="helper-tex-channel-base_url-label"> {inputPrompt.baseUrl} </FormHelperText>
								)}
							</FormControl>
							...
							<FormControl fullWidth error={Boolean(touched.modelMapping && errors.modelMapping)} sx={{ ...theme.typography.otherInput }}>
								{/* <InputLabel htmlFor="channel-model_mapping-label">{inputLabel.modelMapping}</InputLabel> */}
								<TextField
									multiline
									id="channel-model_mapping-label"
									label={inputLabel.modelMapping}
									value={values.modelMapping}
									name="modelMapping"
									onBlur={handleBlur}
									onChange={handleChange}
									aria-describedby="helper-text-channel-model_mapping-label"
									minRows={5}
									placeholder={inputPrompt.modelMapping}
								/>
								{touched.modelMapping && errors.modelMapping ? (
									<FormHelperText error id="helper-tex-channel-model_mapping-label">
										{errors.modelMapping}
									</FormHelperText>
								) : (
									<FormHelperText id="helper-tex-channel-model_mapping-label"> {inputPrompt.modelMapping} </FormHelperText>
								)}
							</FormControl>
							<DialogActions>
