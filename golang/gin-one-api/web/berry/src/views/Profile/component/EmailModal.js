...
const validationSchema = Yup.object().shape({
	...
	emailVerificationCode: Yup.string().required("验证码不能为空"),
});
...
const EmailModal = ({ open, handleClose, turnstileToken }) => {
	...
	const submit = async (values, { setErrors, setStatus, setSubmitting }) => {
		setLoading(true);
		setSubmitting(true);
		const res = await API.get(`/api/oauth/email/bind?email=${values.email}&code=${values.emailVerificationCode}`);
					...
					>
						{({ errors, touched, handleBlur, handleChange, handleSubmit, values }) => (
							<form noValidate onSubmit={handleSubmit}>
									...
									<OutlinedInput
										...
										endAdornment={
											<InputAdornment position="end">
												<Button variant="contained" color="primary" onClick={() => handleSendCode(values.email)} disabled={disableButton || loading}>
								...
								>
									<InputLabel htmlFor="emailVerificationCode">验证码</InputLabel>
									<OutlinedInput
									...
									{touched.emailVerificationCode && errors.emailVerificationCode && (
										<FormHelperText error id="helper-emailVerificationCode">
