...
func PostPushMessage(c *gin.Context) {
	var message model.MessageEntity
	...
	if message == (model.MessageEntity{}) {
		c.JSON(http.StatusOK, gin.H{
			"success": false,
...
func pushMessageHelper(c *gin.Context, message *model.MessageEntity) {
	...
	if user.Status == common.UserStatusUnknown {
		c.JSON(http.StatusOK, gin.H{
			"success": false,
			"message": "用户不存在",
		})
...
func processMessage(c *gin.Context, message *model.MessageEntity, user *model.UserEntity) {
	...
	if message.Channel == "" {
		message.Channel = user.Profile.Channel
		if message.Channel == "" {
			message.Channel = common.TypeEmail
		}
	}
...
func saveAndSendMessage(user *model.UserEntity, message *model.MessageEntity, channel_ *model.ChannelEntity) error {
