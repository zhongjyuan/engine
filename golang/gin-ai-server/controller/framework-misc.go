...
func SendEmailVerification(c *gin.Context) {
	...
	if err := common.Validate.Var(email, "required,email"); err != nil {
		common.SendJSONResponse(c, http.StatusBadRequest, false, "无效的参数！", nil)
		return
	}

	// 检查电子邮件地址是否已被占用
	if model.IsEmailAlreadyTaken(email) {
		common.SendJSONResponse(c, http.StatusBadRequest, false, "邮箱地址已被占用", nil)
		return
	}
	...
	if err != nil {
		common.SendJSONResponse(c, http.StatusBadRequest, false, err.Error(), nil)
		return
	}
...
func SendEmailResetPassword(c *gin.Context) {
	...
	// 检查邮箱地址是否已注册
	if !model.IsEmailAlreadyTaken(email) {
		common.SendJSONResponse(c, http.StatusBadRequest, false, "该邮箱地址未注册", nil)
		return
	}
