...
func auth(c *gin.Context, minRole int) {
	...
	if userName == nil {
		...
		if token == "" {
			common.SendJSONResponse(c, http.StatusUnauthorized, false, "无权进行此操作，未登录或 token 无效", nil)
			c.Abort()
			return
		}
		...
		if user != nil && user.UserName != "" { // Token 有效，设置用户信息
			...
			common.SendJSONResponse(c, http.StatusUnauthorized, false, "无权进行此操作，token 无效", nil)
			c.Abort()
			return
		}
