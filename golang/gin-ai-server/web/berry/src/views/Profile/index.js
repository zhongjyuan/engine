...
export default function Profile() {
	...
	const generateAccessToken = async () => {
		...
		const { success, message, data } = res.data;
		if (success) {
			setInputs((inputs) => ({ ...inputs, accessToken: data }));
	...
	return (
		<>
				...
				<Card sx={{ paddingTop: "20px" }}>
					<Stack spacing={2}>
						<Stack direction="row" alignItems="center" justifyContent="center" spacing={2} sx={{ paddingBottom: "20px" }}>
							<Label variant="ghost" color={inputs.wechatId ? "primary" : "default"}>
								<IconBrandWechat /> {inputs.wechatId || "未绑定"}
							</Label>
							<Label variant="ghost" color={inputs.githubId ? "primary" : "default"}>
								<IconBrandGithub /> {inputs.githubId || "未绑定"}
							</Label>
						...
						<SubCard title="个人信息">
							<Grid container spacing={2}>
								...
								<Grid xs={12}>
									<FormControl fullWidth variant="outlined">
										<InputLabel htmlFor="displayName">显示名称</InputLabel>
										<OutlinedInput
											id="displayName"
											label="显示名称"
											type="text"
							...
							<Grid container spacing={2}>
								...
								{inputs.accessToken && (
									<Grid xs={12}>
										<Alert severity="error">
											你的访问令牌是: <b>{inputs.accessToken}</b> <br />
											请妥善保管。如有泄漏，请立即重置。
										</Alert>
									</Grid>
								)}
								<Grid xs={12}>
									<Button variant="contained" onClick={generateAccessToken}>
										{inputs.accessToken ? "重置访问令牌" : "生成访问令牌"}
									</Button>
								</Grid>
			...
			<Dialog open={showAccountDeleteModal} onClose={() => setShowAccountDeleteModal(false)} maxWidth={"md"}>
				<DialogTitle sx={{ margin: "0px", fontWeight: 500, lineHeight: "1.55556", padding: "24px", fontSize: "1.125rem" }}>危险操作</DialogTitle>
				<Divider />
