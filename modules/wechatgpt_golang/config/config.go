...
func LoadConfig() *Configuration {
	once.Do(func() {
		...
		// 初始化日志记录对象
		logger = wechatbot.NewLogger(LogDir, wechatbot.LogLevel(config.LogLevel))
	})
...
//
// 输入参数：
//   - b *wechatbot.Bot: 要设置的机器人变量。
//
// 输出参数：
//   - 无。
func SetBot(b *wechatbot.Bot) {
	bot = b
}
...
//
// 输出参数：
//   - *wechatbot.Bot: 返回当前的机器人变量。
func GetBot() *wechatbot.Bot {
	return bot
}
...
//
// 输出参数：
//   - *wechatbot.Logger: 返回当前的日志记录器。
func Logger() *wechatbot.Logger {
	return logger
}
