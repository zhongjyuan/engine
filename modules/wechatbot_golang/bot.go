...
func PrintlnQRCodeUrl(bot *Bot, uuid string) {
	...
	// 使用 Open 函数在浏览器中打开登录链接
	_ = Open(qrcodeUrl)
}

// ================================================= [函数](Bot)私有 =================================================

// dumpTo 用于将机器人的状态信息序列化到指定的 Writer 中(注: 写之前最好先清空之前的数据)
//
// 入参：
...
func (b *Bot) syncCheck() error {
	...
	syncCheckOption := &CallerSyncCheckRequest{} // 创建同步检查的配置选项对象

	// 判断是否存活
	for b.Alive() {
		// 重置相关参数，因为它们可能是动态的
		syncCheckOption.BaseRequest = b.Storage.Request
		syncCheckOption.InitResponse = b.Storage.Response
...
func (b *Bot) syncMessage() ([]*Message, error) {
	option := CallerSyncRequest{
		BaseRequest:       b.Storage.Request,   // 设置基本请求参数
		InitResponse:      b.Storage.Response,  // 设置Web初始化响应参数
...
func (b *Bot) Init() error {
	...
	// 构建通知手机客户端已登录的参数
	notifyOption := &CallerStatusNotifyRequest{
		BaseRequest:       req,
		InitResponse:      resp,
...
func (b *Bot) Reload() error {
	...
	if item.SyncKey != nil { // 如果反序列化得到的 SyncKey 不为空
		if b.Storage.Response == nil { // 并且机器人 Storage 中的 Response 为空
			b.Storage.Response = &InitResponse{} // 则创建一个空的 WebInitResponse，用于存储 SyncKey
		}
...
func (b *Bot) DumpHotReloadStorage() error {
	...
	return b.dumpTo(b.hotReloadStorage) // 将当前机器人状态信息保存到热重载存储中
}
