...
//   - *http.Response: HTTP 响应对象，包含登录结果。
//   - error: 错误对象，表示发生的错误。
func (web webMode) PushLogin(ctx context.Context, client *WechatClient, uin int64) (*http.Response, error) {
	// 构造请求路径
...
//   - error: 错误对象，表示发生的错误。
func (web webMode) GetLoginUUID(ctx context.Context, client *WechatClient) (*http.Response, error) {
...
//   - error: 错误对象，表示发生的错误。
func (web webMode) GetLoginInfo(ctx context.Context, client *WechatClient, path string) (*http.Response, error) {
...
//   - *http.Response: HTTP 响应对象，包含获取登录 UUID 的结果。
//   - error: 错误对象，表示发生的错误。
func (desktop desktopMode) GetLoginUUID(ctx context.Context, client *WechatClient) (*http.Response, error) {
	path, err := url.Parse(client.Domain.LoginHost() + jsLoginPath)
	if err != nil {
		return nil, err
	}
...
//   - *http.Response: HTTP 响应对象，包含获取登录信息的结果。
//   - error: 错误对象，表示发生的错误。
func (desktop desktopMode) GetLoginInfo(ctx context.Context, client *WechatClient, path string) (*http.Response, error) {
	req, err := http.NewRequestWithContext(ctx, http.MethodGet, path, nil)
	if err != nil {
		return nil, err
	}
...
//   - error: 错误对象，表示发生的错误。
func (desktop desktopMode) PushLogin(ctx context.Context, client *WechatClient, uin int64) (*http.Response, error) {
	path, err := url.Parse(client.Domain.BaseHost() + pushLoginUrlPath)
	if err != nil {
		return nil, err
	}
