...
export function getConfig(data) {
	let domain = (data && data.Domain) || "wx.qq.com"; // 如果 data.Domain 未定义，则默认设置为 "wx.qq.com"

	let baseHost = `https://${domain}`; // 构建 baseHost 变量，以 https:// 开头，后跟 host 变量的值
	let fileHost = `https://file.${domain}`; // 设置默认的文件 URL
	let pushHost = `https://webpush.${domain}`; // 设置默认的推送 URL
	let loginHost = `https://login.${domain}`; // 设置默认的登录 URL
