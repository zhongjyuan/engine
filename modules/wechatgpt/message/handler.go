...
func Handler(message *core.Message) {
	...
	// 群消息处理[chatgpt]
	dispatcher.OnGroup(func(ctx *core.MessageContext) { handlers[GroupHandler].Handle(ctx.Message) })

	// 添加好友消息处理
	dispatcher.OnFriendAdd(func(ctx *core.MessageContext) { handlers[FriendAddHandler].Handle(ctx.Message) })

	// 文件消息处理
	dispatcher.RegisterHandler(func(message *core.Message) bool { return message.HasFile() }, func(ctx *core.MessageContext) { handlers[FileHandler].Handle(ctx.Message) })
