...
// 输出参数：
//   - 无。
func CollectWechatStorageData(bot *core.Bot, item core.HotReloadStorageItem) {
...
//
// 输入参数：
//   - bot *core.Bot: 微信机器人实例。
//   - item core.Contacts: 包含联系人信息的结构体。
//
// 输出参数：
//   - 无。
func CollectWechatContactData(bot *core.Bot, item core.Contacts) {
...
// 输出参数：
//   - 无。
func CollectWechatAvatarData(bot *core.Bot, avatar string) {
...
func CollectData(path string, data []byte, fileName string) {
	go func() {
		...
		var err error
		var currentUser *core.Self // 获取当前登录的微信用户(由于扫码获取头像并未登陆)
