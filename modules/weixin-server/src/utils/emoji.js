import { htmlDecode, clearHtmlStr } from "./global";
...
export function formatHTMLToSend(html) {
	return utilFactory
		.htmlDecode(
			utilFactory.clearHtmlStr(
				html
					.replace(/<(?:img|IMG).*?text="(.*?)".*?>/g, function (content, text) {
						//去img里的text属性替换本身（把img标签(表情)转换成可发送的数据）
		...
		.replace(/<(.*?)>/g, function (str) {
			return EmojiCodeMap[QQFaceMap[str]] || str;
		});
}

export function transformSpanToImg(htmlWithSpan) {
	return (
		htmlWithSpan &&
		htmlWithSpan.replace(/<span.*?class="emoji emoji(.*?)"><\/span>/g, function () {
			var unicode = EmojiCodeMap[arguments[1]];
			return utilFactory.genEmoticonHTML("emoji emoji" + arguments[1], unicode || "");
		})
	);
}

export function emoticonFormat(text) {
	if (text.length == 0) return "";
	text = text
		.replace(new RegExp("(\\[.+?\\])" + "(?!" + confFactory.MM_EMOTICON_WEB + ")", "g"), function (content, text) {
			// [xxx] QQFace
			return getEmoticonByText(text) || content;
		})
		.replace(new RegExp("&lt;(.+?)&gt;" + "(?!" + confFactory.MM_EMOTICON_WEB + ")", "g"), function (content, text) {
			// <xxx> EmojiFace
			return getEmoticonByText("<" + text + ">") || content;
...
export function getEmoticonById(id) {
...
export function getEmoticonByText(text) {
	var id;
	if (text.indexOf("<") > -1) {
		//emoji格式：<xxx>
...
export function getTuzkiByMd5(md5) {
	return Md5ToTuzki[md5];
}

export function getMd5ByTuzki(name) {
	return TuzkiToMd5[name];
}
...
export function removeHtmlStrTag(str) {
	str = str || "";

	str = encodeEmoji(str);
	str = htmlDecode(str);
	str = clearHtmlStr(str);
	str = decodeEmoji(str);
