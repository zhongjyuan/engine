...
function getConfig(host) {
		...
		var origin = "https://" + host; // 构建 origin 变量，以 https:// 开头，后跟 host 变量的值
		var loginUrl = "login.weixin.qq.com"; // 设置默认的登录 URL
		var fileUrl = "file.wx.qq.com"; // 设置默认的文件 URL
		var pushUrl = "webpush.weixin.qq.com"; // 设置默认的推送 URL

		// 使用正则表达式匹配 host 中的特定部分
		var matchResult = host.match(/(\w+)(.qq.com|.wechat.com)/);
		if (matchResult && matchResult[1] && matchResult[2]) {
				var prefix = matchResult[1]; // 匹配结果的第一个组成部分
				var suffix = matchResult[2]; // 匹配结果的第二个组成部分
				...
				return Object.assign(config, _base2.default); // 将 defaultConfig 对象的属性合并到 config 对象中，并返回合并后的结果
		}
}
