...
/**语言源文件目录 */
const languageFileDir = path.join(__dirname, "../config/langs");
...
function buildLocalization() {
	...
	/**多语言文件正文 */
	let fileContents = JSON.stringify(languages);

	// 添加 localizationHelpers.js 文件的内容（包括帮助函数等）
	// fileContents += fs.readFileSync(path.join(__dirname, "../common/localizationHelper.js"));

	//
	// fileContents += "\n\nif(module.parent){module.exports=languages;}";
