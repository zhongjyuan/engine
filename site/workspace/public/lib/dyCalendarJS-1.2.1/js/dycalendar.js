...
(function (global) {
	...
	var //this will be used by the user.
		...
		monthName = {
			...
			zfull: ["一月", "二月", "三月", "四月", "五月", "六月", "七月", "八月", "九月", "十月", "十一月", "十二月"],
			zmmm: ["一", "二", "三", "四", "五", "六", "七", "八", "九", "十", "十一", "十二"],
		},
		//name of the days
		dayName = {
			...
			zfull: ["星期日", "星期一", "星期二", "星期三", "星期四", "星期五", "星期六"],
			zd: ["日", "一", "二", "三", "四", "五", "六"],
		};
	...
	function createMonthTable(data, option) {
		...
		for (c = 0; c <= 6; c = c + 1) {
            ...
            } else if (option.dayformat === "zfull") {
                td.innerHTML = dayName.zfull[c];
            } else if (option.dayformat === "zd") {
                td.innerHTML = dayName.zd[c];
            }
			tr.appendChild(td);
		}
	...
	function drawCalendarMonthTable(data, option) {
		...
		} else if (option.monthformat === "zfull") {
			elem.innerHTML = data.monthNamezFull + " " + data.year;
		} else if (option.monthformat === "zmmm") {
			elem.innerHTML = data.monthNamezmmm + " " + data.year;
		}
	...
	function drawCalendarDay(data, option) {
		...
		if (option.dayformat === "ddd") {
			...
			elem.innerHTML = dayName.zfull[data.targetedDayIndex];
		} else if (option.dayformat === "zd") {
			elem.innerHTML = dayName.zd[data.targetedDayIndex];
		}
	...
	function getCalendar(year, month, date) {
		...
		result.today.dayzdName = dayName.zd[idx];
		result.today.dayzFullName = dayName.zfull[idx];
		...
		result.today.monthNamezmmm = monthName.zmmm[idx];
		result.today.monthNamezFull = monthName.zfull[idx];
		...
		result.firstDayzdName = dayName.zd[idx];
		result.firstDayzFullName = dayName.zfull[idx];
		...
		result.monthNamezmmm = monthName.zmmm[idx];
		result.monthNamezFull = monthName.zfull[idx];
		...
		result.targetedDayzdName = dayName.zd[idx];
		result.targetedDayzFullName = dayName.zfull[idx];
