...
function sizeHandler() {
	...
	if ($("#canvas")) {
		var a = CANVAS_WIDTH,
			...
			c,
			d;
		!0 === inIframe() && "ios" == getMobileOperatingSystem()
...
function calculateIntersection(a, d) {
	...
	return 0 > b && 0 > c
		? ((b = Math.min(Math.min(a.width, d.width), -b)),
		  ...
		  { x: Math.max(a.x, d.x), y: Math.max(a.y, d.y), width: b, height: c, rect1: a, rect2: d })
		: null;
}
function getBounds(a, d) {
	...
	if (a instanceof createjs.Container) {
		...
		a instanceof createjs.Bitmap
			? ((f = a.sourceRect || a.image), (g = f.width * d), (k = f.height * d))
			...
			? a.spriteSheet._frames && a.spriteSheet._frames[a.currentFrame] && a.spriteSheet._frames[a.currentFrame].image
				? ((f = a.spriteSheet.getFrame(a.currentFrame)), (g = f.rect.width), (k = f.rect.height), (c = f.regX), (l = f.regY))
				: ((b.x = a.x || 0), (b.y = a.y || 0))
			: ((b.x = a.x || 0), (b.y = a.y || 0));
		c = c || 0;
		g = g || 0;
...
(function() {
	function a(a) {
		var c = { focus: "visible", focusin: "visible", pageshow: "visible", blur: "hidden", focusout: "hidden", pagehide: "hidden" };
		a = a || window.event;
		a.type in c
...
function CScoreText(a, d, b) {
	...
	this.moveUp = function() {
		var a = c.y - 100,
			b = this;
		createjs.Tween.get(c)
			.to({ y: a }, 1e3, createjs.Ease.sineIn)
			.call(function() {
				b.unload();
			});
	};
...
function CScope() {
	...
	this.update = function() {
		e && g
			? ((l += SCOPE_ACCELERATION), (h -= SCOPE_ACCELERATION))
			: e && k
			? ((l += SCOPE_ACCELERATION), (h += SCOPE_ACCELERATION))
			: f && k
			? ((l -= SCOPE_ACCELERATION), (h += SCOPE_ACCELERATION))
			...
			? ((l -= SCOPE_ACCELERATION), (h -= SCOPE_ACCELERATION))
			: f
			? (l -= SCOPE_ACCELERATION)
			: e
			? (l += SCOPE_ACCELERATION)
			: g
			? (h -= SCOPE_ACCELERATION)
			: k && (h += SCOPE_ACCELERATION);
		b.x += l;
		b.y += h;
		l *= SCOPE_FRICTION;
		h *= SCOPE_FRICTION;
...
function CMain(a) {
	...
	this.soundLoaded = function() {
		...
		if (b === c) {
			g.unload();
			if (!1 === DISABLE_SOUND_MOBILE || !1 === s_bMobile) s_oSoundTrack = createjs.Sound.play("soundtrack", { loop: 100 });
			this.gotoMenu();
		}
	};
		...
		createjs.Sound.initializeDefaultPlugins() &&
			(0 < navigator.userAgent.indexOf("Opera") || 0 < navigator.userAgent.indexOf("OPR")
				? ((createjs.Sound.alternateExtensions = ["m4a"]),
				  ...
				  createjs.Sound.registerSound("media/audio/ds_duck_hit.ogg", "duck_hit", 5),
				  createjs.Sound.registerSound("media/audio/ds_duck_intro.ogg", "duck_intro", 5),
				  createjs.Sound.registerSound("media/audio/ds_game_over.ogg", "game_over", 5),
				  createjs.Sound.registerSound("media/audio/ds_no_bullets.ogg", "click"),
				  createjs.Sound.registerSound("media/audio/ds_shot.ogg", "shot"),
				  createjs.Sound.registerSound("media/audio/ds_soundtrack.mp3", "soundtrack"))
				: ((createjs.Sound.alternateExtensions = ["ogg"]),
				  createjs.Sound.addEventListener("fileload", createjs.proxy(this.soundLoaded, this)),
				  createjs.Sound.registerSound("media/audio/ds_duck_hit.m4a", "duck_hit", 5),
				  createjs.Sound.registerSound("media/audio/ds_duck_intro.m4a", "duck_intro", 5),
				  createjs.Sound.registerSound("media/audio/ds_game_over.m4a", "game_over", 5),
				  createjs.Sound.registerSound("media/audio/ds_no_bullets.m4a", "click"),
				  createjs.Sound.registerSound("media/audio/ds_shot.m4a", "shot"),
				  createjs.Sound.registerSound("media/audio/ds_soundtrack.mp3", "soundtrack")),
			(c += 6));
	};
	this._loadImages = function() {
		s_oSpriteLibrary.init(this._onImagesLoaded, this._onAllImagesLoaded, this);
		s_oSpriteLibrary.addSprite("but_play", "image/sprites/but_play.png");
		s_oSpriteLibrary.addSprite("but_exit", "image/sprites/but_exit.png");
		s_oSpriteLibrary.addSprite("bg_menu", "image/sprites/bg_menu.jpg");
		s_oSpriteLibrary.addSprite("msg_box", "image/sprites/msg_box.png");
		s_oSpriteLibrary.addSprite("but_left", "image/sprites/but_left.png");
		s_oSpriteLibrary.addSprite("but_right", "image/sprites/but_right.png");
		s_oSpriteLibrary.addSprite("but_up", "image/sprites/but_up.png");
		s_oSpriteLibrary.addSprite("but_down", "image/sprites/but_down.png");
		s_oSpriteLibrary.addSprite("but_upleft", "image/sprites/but_up_left.png");
		s_oSpriteLibrary.addSprite("but_downleft", "image/sprites/but_down_left.png");
		s_oSpriteLibrary.addSprite("but_upright", "image/sprites/but_up_right.png");
		s_oSpriteLibrary.addSprite("but_downright", "image/sprites/but_down_right.png");
		s_oSpriteLibrary.addSprite("shot_panel", "image/sprites/shot_panel.png");
		s_oSpriteLibrary.addSprite("hit_panel", "image/sprites/hit_panel.png");
		s_oSpriteLibrary.addSprite("bullet", "image/sprites/bullet.png");
		s_oSpriteLibrary.addSprite("tap_shot", "image/sprites/tap_shot.png");
		s_oSpriteLibrary.addSprite("hit_icon", "image/sprites/hit_icon.png");
		s_oSpriteLibrary.addSprite("audio_icon", "image/sprites/audio_icon.png");
		s_oSpriteLibrary.addSprite("bg_game", "image/sprites/bg_game.jpg");
		s_oSpriteLibrary.addSprite("grass", "image/sprites/grass.png");
		s_oSpriteLibrary.addSprite("tree", "image/sprites/tree.png");
		s_oSpriteLibrary.addSprite("scope", "image/sprites/gun_sight.png");
		s_oSpriteLibrary.addSprite("tap_shot", "image/sprites/tap_shot.png");
		s_oSpriteLibrary.addSprite("duck_1", "image/sprites/duck_1.png");
		s_oSpriteLibrary.addSprite("duck_2", "image/sprites/duck_2.png");
		s_oSpriteLibrary.addSprite("duck_3", "image/sprites/duck_3.png");
		s_oSpriteLibrary.addSprite("duck_4", "image/sprites/duck_4.png");
		s_oSpriteLibrary.addSprite("target", "image/sprites/target.png");
		s_oSpriteLibrary.addSprite("life_panel", "image/sprites/life_panel.png");
		s_oSpriteLibrary.addSprite("help_panel_desktop", "image/sprites/help_panel_desktop.png");
		s_oSpriteLibrary.addSprite("help_panel_mobile", "image/sprites/help_panel_mobile.png");
...
function CInterface() {
	...
	this._init = function() {
		...
		l = [];
		for (
			var q = s_oSpriteLibrary.getSprite("hit_icon"),
				v = new createjs.SpriteSheet({ images: [q], frames: { width: 51, height: 43 }, animations: { nohit: [0], hit: [1] } }),
				z = 230,
				q = 0;
			9 > q;
			q++
		...
		for (q = 0; q < NUM_BULLETS; q++)
			(w = createBitmap(s_oSpriteLibrary.getSprite("bullet"))), (w.x = v), (w.y = 680), s_oStage.addChild(w), (h[q] = w), (v += 30);
	...
	this.noAmmo = function() {
		g.visible = !0;
		g.alpha = 1;
		createjs.Tween.get(g, { override: !0 }).to({ alpha: 0 }, 1e3, createjs.Ease.quadIn);
	};
...
function CGame(a) {
	...
	this._init = function() {
		...
		s_bMobile
			? window.navigator.msPointerEnabled
				? ((t = 0),
				  s_oCanvas.addEventListener("MSPointerDown", this.onTouchStartMS, !1),
		...
		(!1 !== DISABLE_SOUND_MOBILE && !1 !== s_bMobile) || s_oSoundTrack.setVolume(0.5);
	};
	...
	this.onShot = function() {
		0 === f
			? (A.noAmmo(), (!1 !== DISABLE_SOUND_MOBILE && !1 !== s_bMobile) || createjs.Sound.play("click"))
	...
	this._checkIfDuckHit = function(a) {
		if (
			m.bullsEye().x > a.target().x &&
			m.bullsEye().x < a.target().x + a.target().w &&
			m.bullsEye().y > a.target().y &&
			m.bullsEye().y < a.target().y + a.target().h
		)
			return !0;
	};
	...
	this.checkController = function() {
		...
		H.hitTest(a.x, a.y)
			? (m.moveUp(), m.moveRight(), m.downStop(), m.leftStop())
			  ...
			  I.hitTest(a.x, a.y)
					...
					: ((a = J.globalToLocal(n, r)),
					  J.hitTest(a.x, a.y)
							...
							: ((a = K.globalToLocal(n, r)),
							  K.hitTest(a.x, a.y)
									? (m.moveDown(), m.moveLeft(), m.upStop(), m.rightStop())
									: ((a = D.globalToLocal(n, r)),
									  D.hitTest(a.x, a.y)
											? (m.moveUp(), m.downStop(), m.rightStop(), m.leftStop())
											: ((a = E.globalToLocal(n, r)),
											  E.hitTest(a.x, a.y)
													? (m.moveDown(), m.upStop(), m.rightStop(), m.leftStop())
													: ((a = F.globalToLocal(n, r)),
													  F.hitTest(a.x, a.y)
															? (m.moveLeft(), m.downStop(), m.rightStop(), m.upStop())
															: ((a = G.globalToLocal(n, r)),
															  G.hitTest(a.x, a.y) ? (m.moveRight(), m.downStop(), m.upStop(), m.leftStop()) : m.resetAllDirection())))))));
	};
	this.update = function() {
		if (B) {
...
function CDuck(a) {
	...
	this._calculateMid = function() {
		var a;
		a = t.x > x.x ? new createjs.Point(t.x + 0.2 * (t.x - x.x), x.y - d / 2) : new createjs.Point(x.x - 0.2 * (x.x - t.x), x.y - d / 2);
		h = { start: t, end: x, traj: a };
	};
	...
	this._updateFall = function() {
		createjs.Tween.get(n)
			.to({ y: CANVAS_HEIGHT }, 1e3, createjs.Ease.quadIn)
			.call(function() {
				s_oGame.checkDuck();
			})
			.call(function() {
