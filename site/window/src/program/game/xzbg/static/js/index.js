...
function Block(a, b, d, c, e) {
	...
	this.incrementOpacity = function() {
		if (this.deleted) {
			if (0.925 <= this.opacity) {
				for (var b = this.attachedLane - MainHex.position, b = MainHex.sides - b; 0 > b; ) b += MainHex.sides;
				b %= MainHex.sides;
				MainHex.shakes.push({
					lane: b,
			...
			this.opacity -= 0.075;
			0 >= this.opacity &&
				((this.opacity = 0), (this.deleted = 2), (1 != gameState && 0 != gameState) || localStorage.setItem("saveState", exportSaveState()));
		}
	};
	this.getIndex = function() {
		for (var b = MainHex.blocks[this.attachedLane], a = 0; a < b.length; a++) if (b[a] == this) return a;
	};
	this.draw = function(b, a) {
		...
		this.angle > this.targetAngle
			...
			: this.angle < this.targetAngle && (this.angularVelocity += angularVelocityConst);
		Math.abs(this.angle - this.targetAngle + this.angularVelocity) <= Math.abs(this.angularVelocity)
			? ((this.angle = this.targetAngle), (this.angularVelocity = 0))
			: (this.angle += this.angularVelocity);
		this.width = (2 * this.distFromHex) / Math.sqrt(3);
		this.widthWide = (2 * (this.distFromHex + this.height)) / Math.sqrt(3);
		var d, c, e, v;
		this.initializing
			? ((v = (MainHex.ct - this.ict) / this.initLen),
			  (d = rotatePoint((-this.width / 2) * v, this.height / 2, this.angle)),
			  (c = rotatePoint((this.width / 2) * v, this.height / 2, this.angle)),
			  (e = rotatePoint((this.widthWide / 2) * v, -this.height / 2, this.angle)),
			  (v = rotatePoint((-this.widthWide / 2) * v, -this.height / 2, this.angle)),
			  MainHex.ct - this.ict >= this.initLen && (this.initializing = 0))
			: ((d = rotatePoint(-this.width / 2, this.height / 2, this.angle)),
			  (c = rotatePoint(this.width / 2, this.height / 2, this.angle)),
			  (e = rotatePoint(this.widthWide / 2, -this.height / 2, this.angle)),
			  (v = rotatePoint(-this.widthWide / 2, -this.height / 2, this.angle)));
		this.deleted
			? (ctx.fillStyle = "#FFF")
			: 0 === gameState
			? "r" == this.color.charAt(0)
		...
		var w = trueCanvas.width / 2 + Math.sin((Math.PI / 180) * this.angle) * (this.distFromHex + this.height / 2) + gdx,
			n = trueCanvas.height / 2 - Math.cos((Math.PI / 180) * this.angle) * (this.distFromHex + this.height / 2) + gdy;
		ctx.beginPath();
		...
		this.tint &&
			(1 > this.opacity &&
				((1 != gameState && 0 != gameState) || localStorage.setItem("saveState", exportSaveState()), (this.iter = 2.25), (this.tint = 0)),
			(ctx.fillStyle = "#FFF"),
			...
			ctx.lineTo(w + e.x, n + e.y),
			ctx.lineTo(w + v.x, n + v.y),
			ctx.lineTo(w + d.x, n + d.y),
			ctx.closePath(),
			ctx.fill(),
			(this.tint -= 0.02),
			0 > this.tint && (this.tint = 0));
		ctx.globalAlpha = 1;
	};
}
...
function search(a, b) {
	for (var d = 0; d < a.length; d++) if (a[d][0] == b[0] && a[d][1] == b[1]) return !0;
	return !1;
}
		...
		for (var e = a.blocks[b][d].color, f = -1; 2 > f; f++)
			for (var h = -1; 2 > h; h++)
				if (Math.abs(f) != Math.abs(h)) {
					...
					void 0 !== a.blocks[q] &&
						void 0 !== a.blocks[q][u] &&
						a.blocks[q][u].color == e &&
						!1 === search(c, [q, u]) &&
						0 === a.blocks[q][u].deleted &&
						(c.push([q, u]), floodFill(a, q, u, c));
				}
}
...
function consolidateBlocks(a, b, d) {
	...
	if (!(3 > e.length)) {
		for (b = 0; b < e.length; b++)
			(d = e[b]),
				void 0 !== d && 2 == d.length && (-1 == c.indexOf(d[0]) && c.push(d[0]), (a.blocks[d[0]][d[1]].deleted = 1), f.push(a.blocks[d[0]][d[1]]));
		c = MainHex.ct;
		c - a.lastCombo < settings.comboTime
			? ((settings.comboTime = (1 / settings.creationSpeedModifier) * (waveone.nextGen / 16.666667) * 3),
			  (a.comboMultiplier += 1),
			  (a.lastCombo = c),
			  (c = findCenterOfBlocks(f)),
			  a.texts.push(new Text(c.x, c.y, "x " + a.comboMultiplier.toString(), "bold Q", "#fff", fadeUpAndOut)))
			: ((settings.comboTime = 240), (a.lastCombo = c), (a.comboMultiplier = 1));
...
function calcSide(a, b, d, c) {
	...
	return [
		[e, a],
		[(trueCanvas.width / 2 + c[b][0] - e) * d + e, (trueCanvas.height / 2 + c[b][1] - a) * d + a],
	];
}
...
function Hex(a) {
	...
	this.shake = function(b) {
		...
		gdy += a;
		b.magnitude /= 2;
		if (1 > b.magnitude) for (c = 0; c < this.shakes.length; c++) this.shakes[c] == b && this.shakes.splice(c, 1);
	};
	this.addBlock = function(b) {
		if (1 == gameState || 0 === gameState) {
			...
			b.distFromHex = (MainHex.sideLength / 2) * Math.sqrt(3) + b.height * this.blocks[a].length;
	...
	this.doesBlockCollide = function(b, a, c) {
		b.settled ||
			(void 0 !== a
				? 0 >= a
					? 0 >= b.distFromHex - b.iter * settings.scale - (this.sideLength / 2) * Math.sqrt(3)
						? ((b.distFromHex = (this.sideLength / 2) * Math.sqrt(3)), (b.settled = 1), (b.checked = 1))
						: ((b.settled = 0), (b.iter = 1.5 + (waveone.difficulty / 15) * 3))
					: c[a - 1].settled && 0 >= b.distFromHex - b.iter * settings.scale - c[a - 1].distFromHex - c[a - 1].height
					? ((b.distFromHex = c[a - 1].distFromHex + c[a - 1].height), (b.settled = 1), (b.checked = 1))
					: ((b.settled = 0), (b.iter = 1.5 + (waveone.difficulty / 15) * 3))
				: ((a = this.sides - b.fallingLane),
				  (a += this.position),
				  (a = (a + this.sides) % this.sides),
				  (c = this.blocks[a]),
				  0 < c.length
						? 0 >= b.distFromHex + b.iter * settings.scale - c[c.length - 1].distFromHex - c[c.length - 1].height &&
						  ((b.distFromHex = c[c.length - 1].distFromHex + c[c.length - 1].height), this.addBlock(b))
						: 0 >= b.distFromHex + b.iter * settings.scale - (this.sideLength / 2) * Math.sqrt(3) &&
						  ((b.distFromHex = (this.sideLength / 2) * Math.sqrt(3)), this.addBlock(b))));
	};
	...
	this.draw = function() {
		this.x = trueCanvas.width / 2;
		-2 != gameState && (this.y = trueCanvas.height / 2);
...
function showbottombar() {
	"onLine" in navigator && !navigator.onLine
		? setTimeout(showbottombar, 1e3)
		: ((window.iframeHasLoaded = !0), 1 != gameState && $("#bottombar").fadeIn(150, "linear"));
}
...
function initialize(a) {
	...
	if (1 != a) {
		...
		$(window).unload(function() {
			1 == gameState || -1 == gameState || 0 === gameState
...
function startBtnHandler() {
	setTimeout(function() {
		if ("mobile" == settings.platform) {
			...
			document.body.addEventListener("mousedown", handleClick, !1);
		}
	}, 5);
...
function handleClickTap(a, b, evt) {
	...
	120 > a && 50 > b && $(".helpText").is(":visible")
		? showHelp()
		: 2 == gameState && canRestart
		? init(1)
		: MainHex &&
		  0 !== gameState &&
		  -1 != gameState &&
		  (a < window.innerWidth / 2 &&
				(1 != gameState && -2 != gameState && -1 != gameState && (0 === importing ? resumeGame() : init(1)), MainHex.rotate(1)),
		  a > window.innerWidth / 2 &&
				(1 != gameState && -2 != gameState && -1 != gameState && (0 === importing ? resumeGame() : init(1)), MainHex.rotate(-1)));
}
...
function checkGameOver() {
	for (var a = 0; a < MainHex.sides; a++)
		if (isInfringing(MainHex))
			return (
				-1 == highscores.indexOf(score) && highscores.push(score),
				writeHighScores(),
				gameOverDisplay(),
				(function() {
...
function render() {
	...
	clearGameBoard();
	if (1 === gameState || 2 === gameState || -1 === gameState || 0 === gameState)
		1 > op && (op += 0.01),
			(ctx.globalAlpha = 1),
			drawPolygon(
				trueCanvas.width / 2,
			...
			drawTimer(),
			(ctx.globalAlpha = 1);
	for (a = 0; a < MainHex.blocks.length; a++) for (var b = 0; b < MainHex.blocks[a].length; b++) MainHex.blocks[a][b].draw(!0, b);
	...
	400 > MainHex.ct &&
		0 !== gameState &&
		!MainHex.playThrough &&
		(350 < MainHex.ct && (ctx.globalAlpha = (50 - (MainHex.ct - 350)) / 50),
		50 > MainHex.ct && (ctx.globalAlpha = MainHex.ct / 50),
		renderBeginningText(),
		(ctx.globalAlpha = 1));
	-1 == gameState &&
		((ctx.globalAlpha = 0.9), (ctx.fillStyle = "rgb(236,240,241)"), ctx.fillRect(0, 0, trueCanvas.width, trueCanvas.height), (ctx.globalAlpha = 1));
...
function renderBeginningText() {
	renderText(
		trueCanvas.width / 2 + 1.5 * settings.scale,
		trueCanvas.height / 2 - 208 * settings.scale - 20,
		35,
		"#2c3e50",
		"\u63a7\u5236",
		"35px 微软雅黑"
	...
	renderText(
		trueCanvas.width / 2 - 85 * settings.scale,
		trueCanvas.height / 2 - 169 * settings.scale,
		20,
		"#2c3e50",
		"\u65cb\u8f6c:",
		"20px 微软雅黑"
	...
	/*加提示语*/
	renderText(
		trueCanvas.width / 2 + 0 * settings.scale,
		trueCanvas.height / 2 - 121 * settings.scale + 20,
		16,
		"#2c3e50",
		"\u70b9\u51fb\u5c4f\u5e55\u5de6\u53f3\u6d4b\uff0c\u5411\u5de6\u3001\u5411\u53f3\u65cb\u8f6c\u516d\u8fb9\u5f62\u3002",
		"16px 微软雅黑"
...
function drawKey(a, b, d) {
	...
	switch (a) {
		...
		case "right":
			ctx.font = "20px Fontawesome";
			ctx.translate(b + 12.5 * settings.scale, d + 27.5 * settings.scale);
			ctx.scale(settings.scale, settings.scale);
			ctx.fillText(String.fromCharCode("0xf04b"), 0, 0);
			break;
		default:
			(ctx.font = "35px Roboto"),
				ctx.translate(b + 25 * settings.scale, d + 39.5 * settings.scale),
				ctx.scale(settings.scale, settings.scale),
				ctx.fillText(a, 0, 0);
	}
	ctx.restore();
...
function Text(a, b, d, c, e, f) {
	...
	this.draw = function() {
		return 0 < this.alive
			? ((ctx.globalAlpha = this.opacity), renderText(this.x + gdx, this.y + gdy, 50, this.color, this.text), (ctx.globalAlpha = 1), f(this), !0)
			: !1;
	};
}
...
function update() {
	...
	for (b = 0; b < blocks.length; b++)
		(MainHex.doesBlockCollide(blocks[b]), blocks[b].settled)
			? blocks[b].removed || (blocks[b].removed = 1)
			: blocks[b].initializing || (blocks[b].distFromHex -= blocks[b].iter * settings.scale);
	for (b = 0; b < MainHex.blocks.length; b++)
		for (d = 0; d < MainHex.blocks[b].length; d++)
			1 == MainHex.blocks[b][d].checked &&
				(consolidateBlocks(MainHex, MainHex.blocks[b][d].attachedLane, MainHex.blocks[b][d].getIndex()), (MainHex.blocks[b][d].checked = 0));
	for (b = 0; b < MainHex.blocks.length; b++) {
		a = 99;
		for (d = 0; d < MainHex.blocks[b].length; d++)
			(c = MainHex.blocks[b][d]), 2 == c.deleted && (MainHex.blocks[b].splice(d, 1), blockDestroyed(), d < a && (a = d), d--);
		if (a < MainHex.blocks[b].length) for (d = a; d < MainHex.blocks[b].length; d++) MainHex.blocks[b][d].settled = 0;
	}
	for (b = 0; b < MainHex.blocks.length; b++)
		for (d = 0; d < MainHex.blocks[b].length; d++)
			(c = MainHex.blocks[b][d]),
				MainHex.doesBlockCollide(c, d, MainHex.blocks[b]),
				MainHex.blocks[b][d].settled || (MainHex.blocks[b][d].distFromHex -= c.iter * settings.scale);
...
doudouIcon.src = "image/doudouIcon.png";
function drawScoreboard() {
	1 > scoreOpacity && ((scoreOpacity += 0.01), (textOpacity += 0.01));
	...
	0 === gameState
		? (renderText(
		  ...
		  renderText(
				trueCanvas.width / 2 + gdx + 6 * settings.scale,
				trueCanvas.height / 2 + gdy - 170 * settings.scale,
				150,
				"#2c3e50",
				"\u65cb\u8f6c\u516b\u5366"
		  ),
		  renderText(
				...
				trueCanvas.height / 2 + gdy + 100 * settings.scale,
				20,
				"rgb(44,62,80)",
				"\u5f00\u59cb!"
		  ),
		  ctx.drawImage(
				...
				266,
				120,
				trueCanvas.width / 2 - 120 * settings.scale,
				trueCanvas.height / 2 + 250 * settings.scale,
				266 * settings.scale,
				120 * settings.scale
		  ))
		: (0 != gameState &&
		  ...
		  renderText(trueCanvas.width / 2 + gdx, trueCanvas.height / 2 + gdy, a, "rgb(236, 240, 241)", score));
	ctx.globalAlpha = 1;
}
...
function showText(a) {
	var b = {
		paused:
			"<div class='centeredHeader unselectable'>\u6682\u505c</div><br><div class='unselectable centeredSubHeader'>\u6309 p \u7ee7\u7eed</div><div style='height:100px;line-height:100px;cursor:pointer;'></div>",
		pausedMobile:
			"<div class='centeredHeader unselectable'>\u6682\u505c</div><br><div class='unselectable centeredSubHeader'>\u6309 <i class='fa fa-play'></i> \u7ee7\u7eed</div><div style='height:100px;line-height:100px;cursor:pointer;'></div>",
		start: "<div class='centeredHeader unselectable' style='line-height:80px;'>Press enter to start</div>",
		gameover:
			"<div class='centeredHeader unselectable' style='color: #2C3E50;font-weight: bold;font-size: 3rem;'> Game Over</div><div style='color: #2C3E50;margin-top: 5%;font-size: 2.5rem;'>得分</div><div style='font-size: 6rem;color: #2C3E50;'>" +
			score +
			"</div>",
	};
	...
	if ("gameover" == a) {
		var d = 1,
			...
			e;
		b.gameover +=
			'<div class=\'unselectable centeredSubHeader endBtn\'><a href="###" class=\'moreGame\'>更多游戏</a><a href="###" class="irestartBtn" >重新开始</a></div>';
		if (d) for (c = 0; c < highscores.length; c++) 0 !== highscores[c] && (d = 0);
	}
	$("#overlay").html(b[a]);
...
function blockDestroyed() {
	waveone.nextGen =
		1350 < waveone.nextGen
			? waveone.nextGen - 24 * settings.creationSpeedModifier
			: 600 < waveone.nextGen
			? waveone.nextGen - 10 * settings.creationSpeedModifier
			: 600;
	waveone.difficulty = 35 > waveone.difficulty ? waveone.difficulty + 0.17 * settings.speedModifier : 35;
}
...
function waveGen(a) {
	...
	this.update = function() {
		...
		(this.dt - this.lastGen) * settings.creationSpeedModifier > this.nextGen &&
			600 < this.nextGen &&
			(this.nextGen -= (this.nextGen / 1300) * 10 * settings.creationSpeedModifier);
	};
	this.randomGeneration = function() {
		if (this.dt - this.lastGen > this.nextGen) {
			...
			5 < this.ct &&
				((a = randInt(0, 24)),
				15 < a
					? ((this.ct = 0), (this.currentFunction = this.doubleGeneration))
					: 10 < a
					? ((this.ct = 0), (this.currentFunction = this.crosswiseGeneration))
					: 7 < a
					? ((this.ct = 0), (this.currentFunction = this.spiralGeneration))
					: 4 < a
					? ((this.ct = 0), (this.currentFunction = this.circleGeneration))
					: 1 < a && ((this.ct = 0), (this.currentFunction = this.halfCircleGeneration)));
		}
	};
		...
		35 > this.difficulty &&
			(this.difficulty =
				8 > this.difficulty
					...
					: this.difficulty + ((this.dt - this.last) / 12e7) * settings.speedModifier);
	};
	this.circleGeneration = function() {
		if (this.dt - this.lastGen > this.nextGen + 500) {
			...
			for (c = 0; c < MainHex.sides; c++) addNewBlock(c, d[c % a], 1.5 + (this.difficulty / 15) * 3);
			this.ct += 15;
			this.lastGen = this.dt;
	...
	this.crosswiseGeneration = function() {
		if (this.dt - this.lastGen > this.nextGen) {
			...
			addNewBlock(d, colors[a], 0.6 + (this.difficulty / 15) * 3);
			addNewBlock((d + 3) % MainHex.sides, colors[a], 0.6 + (this.difficulty / 15) * 3);
			this.ct += 1.5;
			this.lastGen = this.dt;
			this.shouldChangePattern();
		}
	...
	this.spiralGeneration = function() {
		var a = randInt(0, 2);
		this.dt - this.lastGen > (2 / 3) * this.nextGen &&
			(a
				? addNewBlock(5 - (this.ct % MainHex.sides), colors[randInt(0, colors.length)], 1.5 + (this.difficulty / 15) * 1.5)
				: addNewBlock(this.ct % MainHex.sides, colors[randInt(0, colors.length)], 1.5 + (this.difficulty / 15) * 1.5),
			(this.ct += 1),
			(this.lastGen = this.dt),
			this.shouldChangePattern());
	};
	...
	this.shouldChangePattern = function(a) {
		if (a)
			switch (((a = randInt(0, 4)), (this.ct = 0), a)) {
				case 0:
...
!(function(a, b) {
	function d() {
		c.READY ||
			...
			g.each(c.gestures, function(a) {
				x.register(a);
			}),
			B.onTouch(c.DOCUMENT, y, x.detect),
			B.onTouch(c.DOCUMENT, r, x.detect),
			(c.READY = !0));
	}
	...
	var g = (c.utils = {
			...
			each: function(a, c, d) {
				...
				else if (a.length !== b) for (l = 0, e = a.length; e > l && !1 !== c.call(d, a[l], l, a); l++);
				else for (l in a) if (a.hasOwnProperty(l) && !1 === c.call(d, a[l], l, a)) break;
			},
			inStr: function(a, b) {
			...
			getDirection: function(a, b) {
				...
				return c >= d ? (0 < a.clientX - b.clientX ? h : u) : 0 < a.clientY - b.clientY ? q : f;
			},
		...
		B = (c.event = {
			...
			onTouch: function(a, b, d) {
				var l = this,
					f = function(e) {
						...
						((F = g.inStr(F, "mouse")) && l.preventMouseEvents) ||
							(F && b == n && 0 === e.button
								? ((l.preventMouseEvents = !1), (l.shouldDetect = !0))
								: h && b == n
								? (l.shouldDetect = 1 === e.buttons)
								: F || b != n || ((l.preventMouseEvents = !0), (l.shouldDetect = !0)),
							h && b != r && C.updatePointer(b, e),
							l.shouldDetect && (f = l.doDetect.call(l, e, b, a, d)),
							f == r && ((l.preventMouseEvents = !1), (l.shouldDetect = !1), C.reset()),
							h && b == r && C.updatePointer(b, e));
					};
			...
			doDetect: function(a, b, c, d) {
				...
				return (
					b != r && d.call(x, a),
					g && ((a.changedLength = k), (a.eventType = g), d.call(x, a), (a.eventType = f), delete a.changedLength),
					f == r && (d.call(x, a), (this.started = !1)),
					f
				);
			},
			determineEventTypes: function() {
				var b;
				return (
					(b = c.HAS_POINTEREVENTS
						? a.PointerEvent
							? ["pointerdown", "pointermove", "pointerup pointercancel lostpointercapture"]
							: ["MSPointerDown", "MSPointerMove", "MSPointerUp MSPointerCancel MSLostPointerCapture"]
						: c.NO_MOUSEEVENTS
			...
			collectEventData: function(a, b, c, d) {
				a = v;
				return (
					g.inStr(d.type, "mouse") || C.matchType(A, d) ? (a = A) : C.matchType(w, d) && (a = w),
					{
		...
		C = (c.PointerEvent = {
			...
			getTouchList: function() {
				var a = [];
				return (
					g.each(this.pointers, function(b) {
						a.push(b);
					}),
					a
				);
			...
			matchType: function(a, b) {
				...
				var c = b.pointerType,
					d = {};
				return (
					(d[A] = c === (b.MSPOINTER_TYPE_MOUSE || A)), (d[v] = c === (b.MSPOINTER_TYPE_TOUCH || v)), (d[w] = c === (b.MSPOINTER_TYPE_PEN || w)), d[a]
				);
			},
			reset: function() {
				this.pointers = {};
		...
		x = (c.detection = {
			...
			getCalculatedData: function(a, b, d, e, f) {
				var m = this.current,
					...
					h = m.lastCalcData;
				k &&
					a.timeStamp - k.timeStamp > c.CALCULATE_INTERVAL &&
					((b = k.center),
					(d = a.timeStamp - k.timeStamp),
					(e = a.center.clientX - k.center.clientX),
					(f = a.center.clientY - k.center.clientY),
					(s = !0));
				(a.eventType != D && a.eventType != z) || (m.futureCalcEvent = a);
				(!m.lastCalcEvent || s) &&
					((h.velocity = g.getVelocity(d, e, f)),
					(h.angle = g.getAngle(b, a.center)),
					(h.direction = g.getDirection(b, a.center)),
					(m.lastCalcEvent = m.futureCalcEvent || a),
					(m.futureCalcEvent = a));
			...
			extendEventData: function(a) {
				var b = this.current,
					c = b.startEvent,
					b = b.lastEvent || c;
				(a.eventType != D && a.eventType != z) ||
					((c.touches = []),
					g.each(a.touches, function(a) {
						c.touches.push({
							clientX: a.clientX,
							clientY: a.clientY,
						});
				...
				return (
					this.getCalculatedData(a, b.center, d, e, f),
					g.extend(a, {
						startEvent: c,
						deltaTime: d,
						deltaX: e,
						...
						scale: g.getScale(c.touches, a.touches),
			...
			register: function(a) {
				var d = a.defaults || {};
				return (
					d[a.name] === b && (d[a.name] = !0),
					g.extend(c.defaults, d, !0),
					(a.index = a.index || 1e3),
	...
	c.Instance = function(a, b) {
		...
		this.options.behavior && g.toggleBehavior(this.element, this.options.behavior, !0);
	...
	c.Instance.prototype = {
		...
		off: function(a, b) {
			var c = this;
			return (
				B.off(c.element, a, b, function(a) {
					a = g.inArray({
						gesture: a,
						handler: b,
					});
	...
	(function(a) {
		...
		c.gestures.Drag = {
			...
			handler: function(c, d) {
				var e = x.current;
				if (!(0 < d.options.dragMaxTouches && c.touches.length > d.options.dragMaxTouches))
					switch (c.eventType) {
						case n:
							b = !1;
							break;
						case y:
							...
							(e.lastEvent.dragLockToAxis || (d.options.dragLockToAxis && d.options.dragLockMinDistance <= c.distance)) && (c.dragLockToAxis = !0);
			...
			handler: function(c, d) {
				...
				switch (c.eventType) {
					...
					case y:
						...
						break;
					case r:
						"touchcancel" != c.srcEvent.type &&
							c.deltaTime < g.tapMaxTime &&
							!b &&
							((e = h && h.lastEvent && c.timeStamp - h.lastEvent.timeStamp),
							(f = !1),
							h && h.name == a && e && e < g.doubleTapInterval && c.distance < g.doubleTapDistance && (d.trigger("doubletap", c), (f = !0)),
							(!f || g.tapAlways) && ((k.name = a), d.trigger(k.name, c)));
				}
			},
			defaults: {
				tapMaxTime: 250,
	...
	c.gestures.Touch = {
		...
		handler: function(a, b) {
			return b.options.preventMouse && a.pointerType == A
				? void a.stopDetect()
				: (b.options.preventDefault && a.preventDefault(), void (a.eventType == D && b.trigger("touch", a)));
		},
	};
	(function(a) {
		var b = !1;
			...
			handler: function(c, d) {
				switch (c.eventType) {
					...
					case y:
						...
						break;
					case z:
						b && 2 > c.changedLength && (d.trigger(a + "end", c), (b = !1));
				}
			},
		};
	})("transform");
...
(function(a) {
	...
	a.clone = function(b, d) {
		return a.parse(a.stringify(b), d);
	};
...
(function() {
	...
	A = function(a, b) {
		...
		d = 0;
		for (e = a.length; d < e; d++) if (((c = a[d]), !(0 <= p.call(b, c)))) return !1;
		return !0;
	};
	v = function(a, b) {
		var c, d, e;
		if (a.length !== b.length) return !1;
		c = d = 0;
		for (e = a.length; 0 <= e ? d < e : d > e; c = 0 <= e ? ++d : --d) if (a[c] !== b[c]) return !1;
		return !0;
	};
	K = function(a, b) {
		var c, d, e;
	...
	J = function(a, b) {
		if (
			(b || keypress.suppress_event_defaults) &&
	...
	r = function(a, b, c) {
		...
		"release" === a && (b.count = 0);
		if ("keyup" === a) return (b.keyup_fired = !0);
	};
	Z = function(a, b) {
		var c, d, e;
		d = 0;
	...
	D = function(a) {
		...
		z(c, function(a) {
			if (!(0 <= p.call(b, a) || a.is_solitary) && q(a.keys)) return b.push(a);
		});
		return b;
	};
	B = function(a) {
	...
	g = function() {
		...
		for (h = t.length; f < h; f++)
			for (a = t[f], b = g = 1, l = E.length; 1 <= l ? g <= l : g >= l; b = 1 <= l ? ++g : --g)
				if (
					((e = E.slice(-b)), a.is_sequence) &&
					(!(0 > p.call(a.keys, "shift")) ||
						((e = y(e, function(a) {
							return "shift" !== a;
						})),
						e.length))
	...
	Y = function(a, b) {
		...
		e = B(a);
		if (e.length) for (f = 0, g = e.length; f < g; f++) (d = e[f]), J(b, d.prevent_default);
	...
	W = function(a, b, c) {
		...
		e = Q(a);
		a.keyup_fired ||
			((d = k.slice()), d.push(c), a.is_solitary && !A(d, a.keys)) ||
			(r("keyup", a, b), a.is_counting && "function" === typeof a.on_keyup && "function" !== typeof a.on_keydown && (a.count += 1));
		e || (r("release", a, b), U(a));
	};
	l = function(a, b) {
		var c, e, f, g, h, l;
		...
		if (1 < e) for (e = 0, g = d.length; e < g; e++) (c = d[e]), void 0 === c || 0 <= p.call(f, c) || Q(c) || U(c);
	};
	M = function(a, b) {
		var c;
		if (L) k.length && (k = []);
		else if (b || k.length) if ((c = w(a.keyCode))) return b ? Y(c, a) : l(c, a);
	};
	V = function(a) {
		var b, c, d, e;
		e = [];
	...
	aa = function(a) {
		var b, c, d, e, f;
		a.keys.length || H("You're trying to bind a combo with no keys.");
		c = e = 0;
		for (f = a.keys.length; 0 <= f ? e < f : e > f; c = 0 <= f ? ++e : --e)
			(d = a.keys[c]),
				(b = F[d]) && (d = a.keys[c] = b),
				"meta" === d && a.keys.splice(c, 1, R),
				"cmd" === d && H('Warning: use the "meta" key rather than "cmd" for Windows compatibility');
		f = a.keys;
		b = 0;
		for (e = f.length; b < e; b++) if (((d = f[b]), 0 > p.call(P, d))) return H('Do not recognize the key "' + d + '"'), !1;
		if (0 <= p.call(a.keys, "meta") || 0 <= p.call(a.keys, "cmd")) {
			b = a.keys.slice();
			e = 0;
			for (f = S.length; e < f; e++) (d = S[e]), -1 < (c = b.indexOf(d)) && b.splice(c, 1);
	...
	for (b in s) (a = s[b]), P.push(a);
	-1 !== navigator.userAgent.indexOf("Mac OS X") && (R = "cmd");
