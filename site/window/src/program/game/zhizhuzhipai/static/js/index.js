...
var pickObj = null;
$("body")
	.on("mousedown", "#pokerBox .pokerLine .pokerLi", function(event) {
	...
	.on("mouseup", function(event) {
		if (pickObj) {
			...
			var range = 0;
			if (endLeft <= leftKb + 120 + 10) {
				// 处于第一列
				range = 1;
			} else if (endLeft >= leftKb + 120 * 9 + 10) {
				// 处于最后一列
				range = 10;
			} else {
...
function initPokerCard() {
	...
	for (var i = 1; i <= pokerAllCount; i++) {
		...
		html +=
			'<div class="pokerLi" isOpen="no" sort="' +
			sort +
			'" card="' +
			card +
			'" style="left: ' +
			...
			card +
			'.png" name="face" />' +
			'<img src="image/f-1.png" name="back" />' +
			"</div>" +
			"</div>";
	}
	...
	setTimeout(function() {
		pokerLineInset = setInterval(function() {
			...
			if (s <= 54) {
				...
				setTimeout(function() {
					for (var i = pokerLineView - 9; i <= pokerLineView; i++) {
						// 执行首次翻牌
...
function gAlert(json) {
	...
	var html =
		'<div class="gAlert" state="now">' +
		'<div class="txt" style="' +
		(json.txtCenter ? "text-align:center;" : "") +
		'">' +
		json.txt +
		"</div>" +
