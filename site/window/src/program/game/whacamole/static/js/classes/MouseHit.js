...
(function() {
	...
	MouseHit.prototype.ready = function(self, index) {
		...
		switch (index) {
			case 0:
				...
				break; //绘制3
			case 1:
				self.context.drawImage(self.startNumber, 390, 296, 54, 87, 360, 300, 54, 87);
				Audio.play("begin_music");
				break; //绘制2
			case 2:
				self.context.drawImage(self.startNumber, 329, 296, 52, 87, 360, 300, 52, 87);
				Audio.play("begin_music");
				break; //绘制1
			case 3:
				self.context.drawImage(self.startNumber, 15, 296, 293, 87, 245, 300, 293, 87);
				Audio.play("second_music");
				break; //绘制start
			case 4:
				clearInterval(this.readyInterval); //清除此Interval事件
	...
	MouseHit.prototype.randomSelectMouse = function(self) {
		...
		for (var i = 1; i <= createNum; i++) {
			...
			if (self.existMatrix[a][b] == 1) {
				...
				if (k == 0) {
					self.mouse0[a][b].state = "normal";
				} else if (k == 1) {
					self.mouse1[a][b].state = "normal";
				} else if (k == 2) {
					self.mouse2[a][b].state = "normal";
				} else if (k == 3) {
					self.mouse3[a][b].state = "normal";
				} else {
					self.mouse4[a][b].state = "normal";
				}
			}
		}
	};
	...
	MouseHit.prototype.drawScreen = function(self) {
		...
		for (j = 0; j < 3; j++) self.drawMouse(self, 0, j); //第一行地鼠
		for (j = 0; j < 3; j++) self.star[0][j].draw(self.context); //第一行星星
		self.context.drawImage(self.bg_holeHide.image, 0, 0, self.bg_holeHide.width, 302, 0, 0, self.bg_holeHide.width, 302); //第一行遮掩洞
		for (j = 0; j < 3; j++) self.drawMouse(self, 1, j); //第二行地鼠
		for (j = 0; j < 3; j++) self.star[1][j].draw(self.context); //第二行星星
		self.context.drawImage(self.bg_holeHide.image, 0, 302, self.bg_holeHide.width, 100, 0, 302, self.bg_holeHide.width, 100); //第二行遮掩洞
		for (j = 0; j < 3; j++) self.drawMouse(self, 2, j); //第三行地鼠
		for (j = 0; j < 3; j++) self.star[2][j].draw(self.context); //第三行星星
		self.context.drawImage(self.bg_holeHide.image, 0, 400, self.bg_holeHide.width, 100, 0, 400, self.bg_holeHide.width, 100); //第三行遮掩洞
		for (i = 0; i < 3; i++) for (j = 0; j < 3; j++) self.scoreObject[i][j].draw(self.context); //分数对象
	...
	MouseHit.prototype.stateInit = function() {
		...
		this.readyInterval = setInterval(function() {
			//setInterval最好是这样写，在控制和传参数方面都有好处
