	...
	var colpick = (function() {
		var tpl = `
				<div class="colpick">
					...
					<div class="colpick_hsx_h colpick_field">
					...
					<div class="colpick_hsx_s colpick_field">
					...
					<div class="colpick_hsx_x colpick_field">
						<div class="colpick_field_letter">B</div>
						<input type="text" maxlength="3" size="3" />
						<div class="colpick_field_arrs">
							<div class="colpick_field_uarr"></div>
		...
		function fillRGBFields(hsx, cal) {
		...
		function fillHSXFields(hsx, cal) {
			$(cal)
				.data("colpick")
				.fields.eq(4)
		...
		function fillHexFields(hsx, cal) {
			$(cal)
				.data("colpick")
				.fields.eq(0)
		...
		function setSelector(hsx, cal) {
			$(cal)
				.data("colpick")
				.selector.css(
					"backgroundColor",
					"#" + ($(cal).data("colpick").hsl ? hslToHex({ h: hsx.h, s: 100, x: 50 }) : hsbToHex({ h: hsx.h, s: 100, x: 100 }))
				);
		...
		function setHue(hsx, cal) {
			$(cal)
				.data("colpick")
				.hue.css("top", parseInt($(cal).data("colpick").height - ($(cal).data("colpick").height * hsx.h) / 360, 10));
		}

		//Set current and new colors
		function setCurrentColor(hsx, cal) {
			$(cal)
				.data("colpick")
				.currentColor.css("backgroundColor", "#" + ($(cal).data("colpick").hsl ? hslToHex(hsx) : hsbToHex(hsx)));
		}

		function setNewColor(hsx, cal) {
			$(cal)
				.data("colpick")
				.newColor.css("backgroundColor", "#" + ($(cal).data("colpick").hsl ? hslToHex(hsx) : hsbToHex(hsx)));
		}
		...
		function upIncrement(ev) {
			change.apply(ev.data.field.get(0), [true]);

			ev.data.el
				.removeClass("colpick_slider")
				.find("input")
				.focus();
		...
		function downHue(ev) {
			...
			change.apply(
				current.cal
					.data("colpick")
					.fields.eq(4)
					.val(parseInt((360 * (current.cal.data("colpick").height - (pageY - current.y))) / current.cal.data("colpick").height, 10))
					.get(0),
		...
		function moveHue(ev) {
			...
			change.apply(
				ev.data.cal
					...
					.val(
						parseInt(
							(360 * (ev.data.cal.data("colpick").height - Math.max(0, Math.min(ev.data.cal.data("colpick").height, pageY - ev.data.y)))) /
		...
		function downSelector(ev) {
			...
			change.apply(
				current.cal
					.data("colpick")
					.fields.eq(6)
					.val(parseInt((100 * (current.cal.data("colpick").height - (pageY - current.pos.top))) / current.cal.data("colpick").height, 10))
					.end()
					.eq(5)
					.val(parseInt((100 * (pageX - current.pos.left)) / current.cal.data("colpick").height, 10))
					.get(0),
				[current.preview]
			);
		...
		function moveSelector(ev) {
			...
			change.apply(
				ev.data.cal
					...
					.val(
						parseInt(
							(100 * (ev.data.cal.data("colpick").height - Math.max(0, Math.min(ev.data.cal.data("colpick").height, pageY - ev.data.pos.top)))) /
								ev.data.cal.data("colpick").height,
							10
						)
					)
					...
					.val(
						parseInt(
							(100 * Math.max(0, Math.min(ev.data.cal.data("colpick").height, pageX - ev.data.pos.left))) / ev.data.cal.data("colpick").height,
							10
						)
					)
					.get(0),
