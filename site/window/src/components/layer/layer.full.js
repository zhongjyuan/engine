...
!(function(window, undefined) {
	...
	var layer = {
		...
		link: function(href, fn, cssname) {
			...
			link.rel = "stylesheet";
			link.href = layer.path.replace("/static", "") + href;
			link.id = id;
		...
		ready: function(callback) {
			var cssname = "skinlayercss",
				ver = "303";
			isLayui
				? layui.addcss("modules/layer/default/layer.css?v=" + layer.v + ver, callback, cssname)
				: layer.link("components/layer/skin/default/layer.css?v=" + layer.v + ver, callback, cssname);
			return this;
		},
	...
	Class.pt.vessel = function(conType, callback) {
		...
		var titleHTML = config.title
			? '<div class="layer-title" style="' + (titype ? config.title[1] : "") + '">' + (titype ? config.title[0] : config.title) + "</div>"
			: "";
		...
		callback(
			[
				...
				'<div class="' +
					...
					((config.type == 0 || config.type == 2) && !config.shade ? " layer-border" : "") +
					" " +
					(config.skin || "") +
					'" id="' +
					doms[0] +
					...
					(config.btn
						? (function() {
								...
								return '<div class="' + doms[6] + (config.btnAlign ? " layer-btn-" + config.btnAlign : "") + '">' + button + "</div>";
						  })()
						: "") +
	...
	layer.prompt = function(options, yes) {
		...
		var prompt,
			content =
				options.formType == 2
					? '<textarea class="layer-input"' + style + ">" + (options.value || "") + "</textarea>"
					: (function() {
							return '<input type="' + (options.formType == 1 ? "password" : "text") + '" class="layer-input" value="' + (options.value || "") + '">';
					  })();
					...
					content:
						...
						(function() {
							var len = tab.length,
								...
								str = "";
							if (len > 0) {
								str = '<li class="layer-tabli xubox_tab_layer">' + (tab[0].content || "no content") + "</li>";
								for (; ii < len; ii++) {
