...
const newPromptList = [...Prompts, ...promptList.value].reduce(
	(acc, { key, value }) => {
		acc[key] = value;
		return acc;
	},
...
promptList.value = Object.entries(newPromptList).map(([key, value]) => ({
	key,
	value,
}));
						...
						<NTabPane name="local" tab="本地管理">
							<NSpace justify="end">
								...
								<NPopconfirm @positive-click="clearPromptTemplate">
									<template #trigger>
										<NButton strong secondary type="default"> 清空 </NButton>
									</template>
									确认是否清空数据?
								</NPopconfirm>
							</NSpace>
									...
									<template #footer>
										<NSpace justify="end">
											...
											<NButton
												...
												@click="setDownloadURL(info.downloadUrl)"
											>
												<NIcon>
													<svg
														xmlns="http://www.w3.org/2000/svg"
